...
var ColorPicker = MediumEditor.Extension.extend({
  ...
  init: function () {
    ...
    this.initVanillaColorPicker(window)
  ...
  initVanillaColorPicker: function (global) {
    function singleColorTpl (color, index, picked) {
      var pickedClass = picked ? 'vanilla-color-picker-single-color-picked' : ''
      return `<div class="vanilla-color-picker-single-color ${pickedClass}" tabindex="${index}" data-color="${color}" style="background-color:${color}" title="${color}"></div>`
    }
    ...
    function SinglePicker (elem, colors, className) {
      ...
      this._positionPickerElement = function () {
        var left = this.targetElem.offsetLeft - 6
