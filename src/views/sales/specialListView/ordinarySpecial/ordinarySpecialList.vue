<template>
  <div :class="$style.main">
    ...
    <el-table
        ...
        stripe
        style="width: 100%">
        <el-table-column prop="id" label="专题ID" width="80"></el-table-column>
        <el-table-column prop="name" label="首页专题名称" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="title" label="专题导航栏标题" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="tags" label="标题下方小字描述" :show-overflow-tooltip="true"></el-table-column>
...
export default {
  ...
  created () {
    ...
    if (page !== 1 && this.$route.params.type === 'ordinary') {
  ...
  methods: {
    changePagin (val) {
      ...
      // this.$router.go({ name: 'specialList', query: { name: '123' }})
      // this.$router.push({ name: 'specialList', params: { type: 'ordinary' }, query: { page: val }})
      this.$router.push({ name: 'specialList', params: { type: 'ordinary' }, query: { page: val }})
    ...
    editSpecial (id) {
      if (!id) {
        this.$router.push({ name: 'specialEdit', params: { type: 'ordinary', operation: 'add' }})
      } else {
        this.$router.push({ name: 'specialEdit', params: { type: 'ordinary', operation: 'edit', id: id }})
      }
    },
    deleteSpecial (item) {
  ...
  computed: {
    pageInfo () {
      return this.$store.state.salesSpecial.ordinarySpecial.list
    }
  },
  watch: {
    'activeTab': function (newVal, oldVal) {
      if (newVal === 'ordinarySpecial') {
        this.$router.push({ name: 'specialList', params: { type: 'ordinary' }, query: { page: this.search.page.page }})        
      }
    }
  }
}
